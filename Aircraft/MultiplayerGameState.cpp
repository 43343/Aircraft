...
bool MultiplayerGameState::handleEvent(const sf::Event& event)
{
	...
	if (event.type == sf::Event::KeyPressed || event.type == sf::Event::JoystickButtonPressed)
	{
		...
		if ((event.key.code == sf::Keyboard::Return || event.joystickButton.button == 0) && mLocalPlayerIdentifiers.size() == 1)
		{
			sf::Packet packet;
		...
		else if (event.key.code == sf::Keyboard::Escape || event.joystickButton.button == 7)
		{
