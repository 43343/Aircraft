...
void Player::handleEvent(const sf::Event& event, CommandQueue& commands)
{
	...
	{
		Action action;
		if (mKeyBinding && (mKeyBinding->checkAction(event.key.code, action) || (event.type == sf::Event::JoystickButtonPressed && mKeyBinding->checkAction(event.joystickButton.button, action))) && !isRealtimeAction(action))
		{
			std::cout << event.joystickButton.button << "j";
	...
	if ((event.type == sf::Event::KeyPressed || event.type == sf::Event::KeyReleased || event.type == sf::Event::JoystickButtonPressed || event.type == sf::Event::JoystickButtonReleased) && mSocket)
	{
		Action action;
		...
		else if (mKeyBinding && mKeyBinding->checkAction(event.joystickButton.button, action) && isRealtimeAction(action))
		{
			// Send realtime change over network
			sf::Packet packet;
			packet << static_cast<sf::Int32>(Client::PlayerRealtimeChange);
