...
SettingsState::SettingsState(StateStack& stack, Context context)
	: State(stack, context)
	, mGUIContainer()
	, mBinaryDataHandler(*context.keys1, *context.keys2)
{
	...
	saveButton->setPosition(80.f, 620.f);
	saveButton->setText("Save");
	saveButton->setCallback([ this]() {
		mBinaryDataHandler.write(mSaveKeyMap);
		requestStackPop();
		});
...
bool SettingsState::handleEvent(const sf::Event& event)
{
	...
	for (std::size_t i = 0; i < 2 * PlayerAction::Count; ++i)
	{
		if (mBindingButtons[i]->isActive())
		{
			...
			{
				std:: cout << keyToString(event.key.code);
				// Player 1
...
void SettingsState::updateLabels()
{
	for (std::size_t i = 0; i < PlayerAction::Count; ++i)
	{
		...
		mBindingLabels[i]->setText(keyToString(key1));
		mBindingLabels[i + PlayerAction::Count]->setText(keyToString(key2));
		mSaveKeyMap[i] = keyToString(key1);
		mSaveKeyMap[i + PlayerAction::Count] = keyToString(key2);
	}
}
